# tests/test_objectstore_azure.py

import pytest
from unittest.mock import patch, MagicMock, mock_open
import backend.data_layer.objectstore_azure as objectstore

@pytest.fixture(autouse=True)
def reset_blob_service_client():
    objectstore._blob_service_client = None

def test_get_azure_blob_config_success():
    with patch('backend.data_layer.objectstore_azure.get_env_variable') as mock_env:
        mock_env.return_value = "UseDevelopmentStorage=true"
        conn_str = objectstore.get_azure_blob_config()
        assert conn_str == "UseDevelopmentStorage=true"

def test_get_azure_blob_config_failure():
    with patch('backend.data_layer.objectstore_azure.get_env_variable', side_effect=Exception("env error")):
        with pytest.raises(Exception):
            objectstore.get_azure_blob_config()

def test_get_blob_service_client_singleton():
    with patch('backend.data_layer.objectstore_azure.get_azure_blob_config') as mock_conf, \
         patch('backend.data_layer.objectstore_azure.BlobServiceClient') as mock_bsc:
        mock_conf.return_value = "conn_str"
        instance = MagicMock()
        mock_bsc.from_connection_string.return_value = instance
        client1 = objectstore.get_blob_service_client()
        client2 = objectstore.get_blob_service_client()
        assert client1 is client2
        mock_bsc.from_connection_string.assert_called_once_with("conn_str")

def test_ensure_container_creates():
    mock_client = MagicMock()
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client):
        objectstore.ensure_container('testcontainer')
        mock_client.create_container.assert_called_with('testcontainer')

def test_ensure_container_exists():
    mock_client = MagicMock()
    mock_client.create_container.side_effect = objectstore.ResourceExistsError("exists")
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client):
        objectstore.ensure_container('testcontainer')
        mock_client.create_container.assert_called_with('testcontainer')
...
def test_upload_file_success():
    ...
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client), \
         patch('backend.data_layer.objectstore_azure.ensure_container'), \
         patch("builtins.open", mock_open(read_data=b"data")) as m:
        objectstore.upload_file('container', 'blob', '/tmp/file')
...
def test_download_file_success():
    ...
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client), \
        ...
        objectstore.download_file('container', 'blob', '/tmp/file')
        mock_client.get_blob_client.assert_called_with(container='container', blob='blob')
...
def test_download_file_not_found():
    mock_client = MagicMock()
    mock_blob_client = MagicMock()
    mock_blob_client.download_blob.side_effect = objectstore.ResourceNotFoundError("not found")
    mock_client.get_blob_client.return_value = mock_blob_client
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client), \
         patch("builtins.open", mock_open()):
        with pytest.raises(objectstore.ResourceNotFoundError):
            objectstore.download_file('container', 'blob', '/tmp/file')
...
def test_list_files_success():
    ...
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client):
        files = objectstore.list_files('container')
...
def test_list_files_failure():
    ...
    with patch('backend.data_layer.objectstore_azure.get_blob_service_client', return_value=mock_client):
        with pytest.raises(Exception):
            objectstore.list_files('container')
